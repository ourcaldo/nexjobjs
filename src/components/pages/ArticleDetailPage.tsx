...
const ArticleDetailPage: React.FC<ArticleDetailPageProps> = ({ article, relatedArticles, settings }) => {
  ...
  useEffect(() => {
    const category = article.categories_info?.[0]?.name || '';
    trackPageView({
      page_title: article.title.rendered,
      content_group1: 'article_detail',
      content_group2: category,
    });

    trackArticleRead(article.title.rendered, article.id.toString(), category);
  }, [article, trackPageView, trackArticleRead]);
  ...
  const getPageTitle = () => {
    return article.seo_title || `${article.title.rendered} - Nexjob`;
  };
  ...
  const getCanonicalUrl = () => {
    return `${getCurrentUrl()}/artikel/${router.query.slug}/`;
  };
...
export async function getServerSideProps(context: any) {
  ...
  const settings = {
    ...
    default_article_og_image: process.env.NEXT_PUBLIC_DEFAULT_ARTICLE_OG_IMAGE
  }
